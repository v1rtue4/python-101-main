...
# set variable for story. """ triple quotes since this story has double quotes. 
...
# Store letters of the current word
current_word = ""

# Loop through each character in story
for char in story:
    if char.isalpha(): # if letter add to current word. 
        current_word += char
    else:
        if current_word:
            # convert word to pig latin
            pig_word = current_word[1:] + current_word[0] + "ay"
            pig_latin_story += pig_word
            current_word = "" # Reset word for next one.


        pig_latin_story += char

# If there's a word left at the very end.
if current_word:
    pig_word = current_word[1:] + current_word[0] + "ay"
    pig_latin_story += pig_word
